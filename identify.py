...
class Identify():
  ...
  def ParseResponse(self, data):
	...
	if self.VendorID in vendorList.keys():
		self.Vendor=vendorList[self.VendorID]
	else:
		self.Vendor="Unknown Vendor"
		
	self.DeviceID=unpack_from('<H', data, 50)[0]
	if self.DeviceID in deviceList.keys():
	  self.DeviceType=deviceList[self.DeviceID]
	else:
...
def ListIdentity():
  ...
  if len(devices)==0:
	  ...
	  try:
	    while(1):
	      ret=s.recv(1024)
	      context=unpack_from('<QDeviceID', ret, 14)[0]
	      if context==0x65696c796168:
		# the data came from our request
		devices.append(Identify().ParseResponse(ret))
	  except:
